...
We developed an interactive demo which uses Viser to visualize a mesh (PLY) and segment it:

```
python scripts/demo.py --mesh=/path/to/mesh.ply --config=/path/to/pretrained/config.yaml --ckpt=/path/to/pretrained/ckpt.pth
```
...
Note: the demo assumes that the mesh is oriented with the up direction as +Z and gravity (down-vector) as -Z, as done in ScanNet40.
